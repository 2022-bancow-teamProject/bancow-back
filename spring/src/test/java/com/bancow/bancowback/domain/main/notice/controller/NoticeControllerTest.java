			...
			.andDo(
				restDocs.document(
					...
					requestFields(
						...
						fieldWithPath("username").description("전화번호"),
					...
					responseFields(
						fieldWithPath("data").description("결과 데이터"),
						fieldWithPath("data.id").description("아이디"),
						fieldWithPath("data.noticeCategory").description("문의 카테고리 (제휴, 투자, 기타)"),
						fieldWithPath("data.username").description("유저이름"),
						fieldWithPath("data.title").description("제목"),
						fieldWithPath("data.message").description("메시지"),
						fieldWithPath("data.status").description("동의 여부"),
						fieldWithPath("data.createDate").description("글 등록일"),
						fieldWithPath("data.updateDate").description("글 수정일"),
						fieldWithPath("status").description("HTTP Status")
					)
				)
